...
Runs inside Termux on Android. `ask-gemini.js` is a single-file CLI with separated concerns: calling Gemini via the `@google/genai` SDK (`ai.models.generateContent({ model, contents })`), capturing a spoken prompt via Termux:API's `termux-speech-to-text` (`captureVoicePrompt`, used when the `--voice` flag is passed), confirming a voice transcript with the user before sending it (`confirmPromptWithUser`), displaying the result (stdout plus a `termux-notification` shell-out that degrades gracefully if unavailable), and, in `--voice` mode only, speaking the reply aloud via Termux:API's `termux-tts-speak` (`speakResponseAloud`, which likewise degrades gracefully if unavailable). The API key is loaded from a gitignored `.env` file via `dotenv`, never hardcoded or logged. `shortcuts/ask-gemini-voice.sh` is a Termux:Widget shortcut script that runs `node ask-gemini.js --voice`, deployed with `cp` (not symlinked, see the Termux:Widget gotcha above) into `~/.shortcuts/` on the phone for a one-tap, no-typing voice query from the home screen. This is the foundation for giving the assistant broader control over the phone in future iterations — document new components here as they're added.

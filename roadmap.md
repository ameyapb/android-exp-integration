...
**Status (2026-09-15): both sub-phases implemented and committed to `main`. Phase 2 is complete.** Sub-phase 2a (voice capture, confirm dialog) and sub-phase 2b (spoken replies) are both built. Voice capture (`data/voice/VoiceRecognizer`/`VoiceRecognizerImpl`, wrapping `android.speech.SpeechRecognizer`), the mic button with on-demand `RECORD_AUDIO` permission request, and the in-app confirm/discard `AlertDialog` send a confirmed transcript through the existing Gemini path and show/notify the reply exactly like a typed prompt. `data/voice/GeminiSpeaker`/`GeminiSpeakerImpl` wraps `android.speech.tts.TextToSpeech`; `AskGeminiViewModel`'s shared `sendPromptAndHandleResult` takes a `speakReplyAloud: Boolean` so only voice-confirmed replies call `geminiSpeaker.speak(replyText)`, never typed ones. Covered by tests: `VoiceRecognizerImplTest` and `GeminiSpeakerImplTest` under Robolectric, plus expanded `AskGeminiViewModelTest` cases (typed send never speaks; voice-confirmed send speaks). `./gradlew build lint testDebugUnitTest` passes with zero lint issues.
